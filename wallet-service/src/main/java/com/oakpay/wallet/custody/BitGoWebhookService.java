...
public class BitGoWebhookService {
    ...
    public BitGoWebhookService(BitGoCustodyProvider bitGo, DepositService depositService,
                               CustodyWebhookEventService eventService, ObjectMapper objectMapper,
    ...
    @Transactional
    public DepositDtos.DepositResponse processTransferNotification(String coinFromPath, String signature, String rawBody) {
        ...
        if (eventId == null) throw new IllegalArgumentException("BitGo webhook notification has no event ID");
        ...
        if (!"receive".equalsIgnoreCase(type)) throw new IllegalArgumentException("BitGo transfer is not a deposit");

        String state = text(transfer, "state");
        if ("failed".equalsIgnoreCase(state) || "rejected".equalsIgnoreCase(state)
                || "removed".equalsIgnoreCase(state) || "replaced".equalsIgnoreCase(state)) {
        ...
        if (valueString == null) throw new IllegalArgumentException("BitGo transfer entry has no value");
        ...
        if ("btc".equalsIgnoreCase(coin)) {
            currency = "BTC"; network = "BITCOIN"; decimals = 8; requiredConfirmations = btcRequiredConfirmations;
        } else if (normalizeCoin(coin).contains("usdt") && normalizeCoin(coin).contains("trx")) {
            currency = "USDT"; network = "TRON"; decimals = 6; requiredConfirmations = usdtTronRequiredConfirmations;
        } else {
            throw new IllegalArgumentException("Unsupported BitGo deposit coin: " + coin);
        }

        BigDecimal amount = new BigDecimal(valueString).movePointLeft(decimals)
        ...
        DepositDtos.BlockchainDepositWebhook normalizedPayload =
                new DepositDtos.BlockchainDepositWebhook(network, address, txHash, currency, amount,
    ...
    private JsonNode readJson(String rawBody) {
        try { return objectMapper.readTree(rawBody); }
        catch (Exception e) { throw new IllegalArgumentException("Invalid BitGo webhook JSON", e); }
    }
