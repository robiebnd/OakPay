...
public class InternalWalletService {
    ...
    public void reserveAdvertisement(UUID userId, String currency, InternalWalletDtos.ReservationRequest request) {
        ...
        if (existing != null) {
            ...
            if (existing.getReservedAmount().compareTo(amount) != 0 && existing.getStatus() == AdvertisementReservationStatus.RESERVED)
                throw new IllegalStateException("Advertisement reservation amount does not match reference");
            return;
        }
    ...
    public void settle(InternalWalletDtos.SettlementRequest request) {
        ...
        Wallet sellerBase = wallet(request.sellerId(), base), sellerQuote = wallet(request.sellerId(), quote);
    ...
    private void validateReservationIdentity(AdvertisementReservation reservation, UUID userId, String currency) {
        if (!reservation.getUserId().equals(userId) || !reservation.getCurrency().equals(currency)) throw new IllegalArgumentException("Advertisement reservation does not belong to this wallet");
    }
    ...
    private void addLedger(Wallet wallet, UUID userId, LedgerTransactionType type, BigDecimal amount, String reference, String metadata) {
        ...
        LedgerEntry entry = new LedgerEntry(); entry.setWalletId(wallet.getId()); entry.setUserId(userId); entry.setTransactionType(type); entry.setStatus(LedgerStatus.COMPLETED); entry.setCurrency(wallet.getCurrency()); entry.setAmount(amount); entry.setBalanceBefore(before.subtract(amount)); entry.setBalanceAfter(before); entry.setReference(reference); entry.setMetadata(metadata); ledgerRepository.save(entry);
    }
