...
public class InternalWalletService {
    ...
    public void lock(UUID userId, String currency, InternalWalletDtos.MutationRequest request) {
        ...
        if (!walletOperationService.begin("LOCK", request.reference(), userId, normalize(currency), amount, request.toString())) return;
    ...
    public void unlock(UUID userId, String currency, InternalWalletDtos.MutationRequest request) {
        Wallet wallet = wallet(userId, currency);
        BigDecimal amount = positive(request.amount());
        if (!walletOperationService.begin("UNLOCK", request.reference(), userId, normalize(currency), amount, request.toString())) return;
    ...
    public void reserveAdvertisement(UUID userId, String currency, InternalWalletDtos.ReservationRequest request) {
        ...
        if (existing != null) {
            ...
            // A cancelled/expired SELL trade was already backed by the advertisement's
            // wallet lock. Restoring it only restores reservation capacity; it must not
            // move available balance into locked balance a second time.
            ...
            if (existing.getStatus() == AdvertisementReservationStatus.CONSUMED) {
    ...
    public void consumeAdvertisementReservation(UUID userId, String currency, InternalWalletDtos.ReservationRequest request) {
        validateReference(request);
        if (request.operationReference() == null || request.operationReference().isBlank()) throw new IllegalArgumentException("Reservation operation reference is required");
    ...
    public void releaseAdvertisementReservation(UUID userId, String currency, InternalWalletDtos.ReservationRequest request) {
        ...
        Wallet wallet = wallet(userId, currency);
        // If an older reservation state says more is reserved than is actually
        // locked in the wallet, release the wallet-backed amount and close the
        // stale reservation instead of making an ad impossible to close.
        BigDecimal releasable = amount.min(wallet.getLockedBalance());
        if (releasable.signum() > 0) {
            wallet.setLockedBalance(wallet.getLockedBalance().subtract(releasable));
            wallet.setAvailableBalance(wallet.getAvailableBalance().add(releasable));
            walletRepository.save(wallet);
        }
    ...
    public void settle(InternalWalletDtos.SettlementRequest request) {
        ...
        walletRepository.save(buyerQuote); walletRepository.save(sellerQuote); walletRepository.save(buyerBase); walletRepository.save(sellerBase);
        ...
        if (buyerFee.signum() > 0) addLedger(buyerQuote, request.buyerId(), LedgerTransactionType.FEE, buyerFee, ref + ":BUYER_FEE", "P2P trading fee");
        if (sellerFee.signum() > 0) addLedger(sellerQuote, request.sellerId(), LedgerTransactionType.FEE, sellerFee, ref + ":SELLER_FEE", "P2P trading fee");
    }

    private AdvertisementReservation reservation(InternalWalletDtos.ReservationRequest request) {
        return reservationRepository.findByReferenceForUpdate(request.reference()).orElseThrow(() -> new IllegalArgumentException("Advertisement reservation not found"));
    }
    private void validateReference(InternalWalletDtos.ReservationRequest request) {
        if (request == null || request.reference() == null || request.reference().isBlank()) throw new IllegalArgumentException("Advertisement reservation reference is required");
    }
    private void validateReservationIdentity(AdvertisementReservation r, UUID userId, String currency) {
        if (!r.getUserId().equals(userId) || !r.getCurrency().equals(currency)) throw new IllegalArgumentException("Advertisement reservation does not belong to this wallet");
    }
    private void addLedger(Wallet wallet, UUID userId, LedgerTransactionType type, BigDecimal amount, String reference, String metadata) {
    ...
    private void validateSettlement(InternalWalletDtos.SettlementRequest r) {
        if (r.buyerId() == null || r.sellerId() == null || r.buyerId().equals(r.sellerId())) throw new IllegalArgumentException("Buyer and seller must be different users");
        if (r.reference() == null || r.reference().isBlank()) throw new IllegalArgumentException("Reference is required");
        if (r.reference().trim().length() > 100) throw new IllegalArgumentException("Reference must not exceed 100 characters");
    }
