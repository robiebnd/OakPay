...
public class P2PDisputeService {
    ...
    @Transactional
    public P2PDisputeDtos.DisputeResponse open(
            UUID userId,
            UUID tradeId,
            P2PDisputeDtos.OpenRequest request) {

        P2PTrade trade = tradeRepository.findById(tradeId)
                .orElseThrow(() ->
                        new IllegalArgumentException("P2P trade not found"));

        if (!trade.getBuyerId().equals(userId)
                && !trade.getSellerId().equals(userId)) {

            throw new IllegalArgumentException(
                    "Trade does not belong to user");
        }

        if (trade.getStatus() != P2PTradeStatus.PAYMENT_MARKED) {

            throw new IllegalStateException(
                    "Only a payment-marked trade can be disputed");
        }
    ...
    public List<P2PDisputeDtos.DisputeResponse> mine(UUID userId) {
        ...
        return disputeRepository
                .findAllByOpenedByOrderByCreatedAtDesc(userId)
                .stream()
                .map(P2PDisputeDtos.DisputeResponse::from)
                .toList();
    }
    ...
    @Transactional(readOnly = true)
    public List<P2PDisputeDtos.DisputeResponse> openDisputes() {

        return disputeRepository
                .findAllByStatusOrderByCreatedAtAsc(DisputeStatus.OPEN)
                .stream()
                .map(P2PDisputeDtos.DisputeResponse::from)
                .toList();
    }
    ...
    public List<P2PDisputeDtos.AuditResponse> audit(
        ...
        return auditRepository
                .findAllByDisputeIdOrderByCreatedAtAsc(disputeId)
                .stream()
                .map(P2PDisputeDtos.AuditResponse::from)
                .toList();
    }
    ...
    public P2PDisputeDtos.DisputeResponse resolve(
            UUID adminId,
            UUID disputeId,
            P2PDisputeDtos.ResolveRequest request) {
        ...
        P2PTrade trade = tradeRepository
                .findById(dispute.getTradeId())
                .orElseThrow(() ->
                        new IllegalArgumentException(
                                "P2P trade not found"));
        ...
        if (trade.getStatus() != P2PTradeStatus.DISPUTED) {
            ...
            throw new IllegalStateException(
                    "Trade is not in dispute status");
        }

        /*
         * BUYER WINS
         *
         * Release the seller's escrowed crypto
         * to the buyer.
         */

        if (request.resolution()
                == DisputeResolution.BUYER_WINS) {
             ...
             * SELLER WINS / CANCEL
             *
             * Return the seller's crypto from escrow
             * and restore the advertisement.
             */
    ...
    private void restoreAdvertisement(P2PTrade trade) {
        ...
        Advertisement ad = advertisementRepository
                .findByIdForUpdate(trade.getAdvertisementId())
                .orElse(null);
        ...
        BigDecimal restored =
                ad.getAvailableQuantity()
                        .add(trade.getQuantity());

        ad.setAvailableQuantity(
                restored.min(ad.getTotalQuantity())
        );

        if (ad.getStatus() == AdStatus.CLOSED
                && ad.getAvailableQuantity().signum() > 0) {
