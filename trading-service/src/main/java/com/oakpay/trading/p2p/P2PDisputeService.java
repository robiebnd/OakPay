...
public class P2PDisputeService {
    ...
    public List<P2PDisputeDtos.DisputeResponse> mine(UUID userId) {
        return disputeRepository.findAllByOpenedByOrderByCreatedAtDesc(userId)
                .stream().map(P2PDisputeDtos.DisputeResponse::from).toList();
    }

    @Transactional(readOnly = true)
    public List<P2PDisputeDtos.DisputeResponse> openDisputes() {
        return disputeRepository.findAllByStatusOrderByCreatedAtAsc(DisputeStatus.OPEN)
                .stream().map(P2PDisputeDtos.DisputeResponse::from).toList();
    }

    @Transactional(readOnly = true)
    public P2PDisputeDtos.DisputeResponse get(UUID disputeId) {
    ...
    @Transactional(readOnly = true)
    public List<P2PDisputeDtos.AuditResponse> audit(UUID disputeId) {
        return auditRepository.findAllByDisputeIdOrderByCreatedAtAsc(disputeId)
                .stream().map(P2PDisputeDtos.AuditResponse::from).toList();
    }

    @Transactional
    public P2PDisputeDtos.DisputeResponse resolve(UUID adminId, UUID disputeId, P2PDisputeDtos.ResolveRequest request) {
    ...
    private void restoreAdvertisement(P2PTrade trade) {
        ...
        if (ad.getStatus() == AdStatus.CLOSED && ad.getAvailableQuantity().signum() > 0) {
