...
public class WalletClient {
    ...
    public void unlock(UUID userId, String currency, BigDecimal amount, UUID referenceId) { unlock(userId,currency,amount,referenceId,null); }
    public void unlock(UUID userId, String currency, BigDecimal amount, UUID referenceId, UUID actorId) { var request=client.post().uri("/api/v1/wallets/{currency}/unlock", currency).contentType(MediaType.APPLICATION_JSON).header("X-OakPay-Internal-Secret", internalSecret); if(actorId!=null) request.header("X-OakPay-Admin-Actor",actorId.toString()); request.body(new WalletMutation(userId, amount, referenceId.toString())).retrieve().toBodilessEntity(); }
    ...
    public void releaseEscrow(UUID sellerId, UUID buyerId, String asset, BigDecimal amount, UUID tradeId) { releaseEscrow(sellerId,buyerId,asset,amount,tradeId,null); }
    public void releaseEscrow(UUID sellerId, UUID buyerId, String asset, BigDecimal amount, UUID tradeId, UUID actorId) { var request=client.post().uri("/api/v1/wallets/internal/escrow/release").contentType(MediaType.APPLICATION_JSON).header("X-OakPay-Internal-Secret", internalSecret); if(actorId!=null) request.header("X-OakPay-Admin-Actor",actorId.toString()); request.body(new EscrowRelease(sellerId,buyerId,asset,amount,tradeId.toString())).retrieve().toBodilessEntity(); }
