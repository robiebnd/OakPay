...
export async function uploadKycDocument(
  ...
  const send = async (accessToken: string) => {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/kyc/documents/${documentId}/upload?side=${side}`, {
      method: 'POST',
    ...
    if (!response.ok) {
      throw new OakPayApiError(response.status, body?.message ?? body?.error ?? `Upload failed with status ${response.status}`);
    }
    return body;
  };
  try {
