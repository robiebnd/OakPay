...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw { status: response.status, message: data.message ?? data.error ?? (typeof body === 'string' ? body : `Request failed with status ${response.status}`), fieldErrors: data.fieldErrors } satisfies ApiError;
    }
    return body as T;
  } catch (error) {
...
export type LedgerTransaction = { id: string; walletId: string; userId: string; currency: string; type: string; amount: number; balanceAfter: number; referenceType?: string; referenceId?: string; description?: string; createdAt: string };
...
export type P2PTrade = { id: string; advertisementId?: string; sellerId?: string; buyerId?: string; asset?: string; fiatCurrency?: string; quantity?: number; price?: number; totalFiatAmount?: number; status?: string; createdAt?: string; updatedAt?: string };
...
export const walletApi = {
  ...
  deposit: (token: string, currency: string, amount: number) => apiPost<LedgerTransaction>(`/api/v1/wallets/${currency}/deposit`, token, { amount }),
  withdraw: (token: string, currency: string, amount: number) => apiPost<LedgerTransaction>(`/api/v1/wallets/${currency}/withdraw`, token, { amount }),
};

export const p2pApi = {
  ads: (token: string, side: 'BUY' | 'SELL', asset = 'USDT', fiat = 'ZWG') => apiGet<P2PAd[]>(`/api/v1/p2p/ads?side=${side}&asset=${asset}&fiatCurrency=${fiat}&limit=50`, token),
