...
export default function PersonalDetailsScreen(){
 ...
 async function save(){if(!accessToken)return;if(!firstName.trim()||!lastName.trim()){Alert.alert('Missing information','First name and last name are required.');return;}setSaving(true);try{const updated=await authApi.updateProfile(accessToken,{firstName:firstName.trim(),lastName:lastName.trim(),phoneNumber:phoneNumber.trim(),country:country.trim(),dateOfBirth:dateOfBirth.trim()||null});setProfile(updated);Alert.alert('Profile updated','Your personal details have been saved.');router.back();}catch(e){Alert.alert('Update failed',e instanceof Error?e.message:'We could not update your profile.');}finally{setSaving(false);}}
 const Field=({label,value,onChangeText,placeholder,keyboardType='default'}:{label:string;value:string;onChangeText:(v:string)=>void;placeholder:string;keyboardType?:'default'|'phone-pad'})=><View style={styles.field}><Text style={styles.label}>{label}</Text><TextInput value={value} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor={MUTED} keyboardType={keyboardType} style={styles.input}/></View>;
 return <SafeAreaView style={styles.safe} edges={['top']}><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}><View style={styles.container}>
