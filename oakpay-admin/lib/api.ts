...
async function rawJson<T>(path:string,init:RequestInit={}):Promise<Response>{
  return fetch(`${BASE}${path}`,{...init,headers:{Accept:'application/json','Content-Type':'application/json',...(init.headers??{})}});
}
...
async function refreshAccessToken():Promise<string|null>{
  ...
  try{
    ...
    if(!response.ok){session.clear();return null;}
...
async function json<T>(path:string,init:RequestInit={}):Promise<T>{
  ...
  if(response.status===401 && init.headers && 'Authorization' in init.headers){
    ...
    if(token){
      ...
      response=await rawJson<T>(path,{...init,headers});
    }
  }
...
export const session={
  get:()=>typeof window==='undefined'?null:localStorage.getItem('oakpay.admin.accessToken'),
  getRefresh:()=>typeof window==='undefined'?null:localStorage.getItem('oakpay.admin.refreshToken'),
  set:(t:TokenResponse)=>{localStorage.setItem('oakpay.admin.accessToken',t.accessToken);if(t.refreshToken)localStorage.setItem('oakpay.admin.refreshToken',t.refreshToken)},
  clear:()=>{localStorage.removeItem('oakpay.admin.accessToken');localStorage.removeItem('oakpay.admin.refreshToken')}
};
