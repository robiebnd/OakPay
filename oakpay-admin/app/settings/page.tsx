...
function SettingCard({icon,title,description,on,onChange,locked=false}:{icon:React.ReactNode;title:string;description:string;on?:boolean;onChange?:(v:boolean)=>void;locked?:boolean}){return <section className="rounded-2xl border border-[#e3e8e5] bg-white p-6 shadow-sm"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#edf5ee] text-[#397b0a]">{icon}</div><div className="mt-5 flex items-start justify-between gap-4"><div><h2 className="text-base font-extrabold text-[#111827]">{title}</h2><p className="mt-2 text-sm leading-6 text-[#6b7280]">{description}</p></div>{locked?<span className="rounded-full bg-[#f3f5f4] px-3 py-1 text-xs font-bold text-[#6b7280]">Backend enforced</span>:<button onClick={()=>onChange?.(!on)} className={`relative mt-1 h-6 w-11 rounded-full transition ${on?"bg-[#397b0a]":"bg-[#cbd5d0]"}`} aria-label={`Toggle ${title}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${on?"left-6":"left-1"}`}/></button>}</div></section>}
