...
public class AuthService {
    ...
    @Transactional
    public AuthDtos.RegistrationResponse register(
    ...
    @Transactional
    public AuthDtos.VerificationResponse verifyEmail(
        ...
        if (
                pending.getVerificationExpiresAt()
                        .isBefore(LocalDateTime.now())
                        || !sha256(request.code().trim())
                        .equals(pending.getVerificationCodeHash())
        ) {
            throw new IllegalArgumentException(
                    "Invalid or expired verification code"
            );
        ...
        if (userRepository.existsByEmailIgnoreCase(email)) {
            ...
            return new AuthDtos.VerificationResponse(
         ...
         *
         * This is explicitly set here rather than relying only
         * on the database default so the application behaviour
         * is clear.
         */
        user.setStatus(ACCOUNT_STATUS_ACTIVE);
    ...
    @Transactional
    public AuthDtos.VerificationResponse resendVerification(
            AuthDtos.ResendVerificationRequest request
    ) {
        String email = normalizeEmail(request.email());
        ...
        enforceCooldown(
                VERIFY_RESEND_PREFIX + sha256(email)
        );
    ...
    public AuthDtos.PasswordResetResponse forgotPassword(
        ...
        if (
            ...
            return new AuthDtos.PasswordResetResponse(
        ...
        enforceCooldown(
                RESET_RESEND_PREFIX + sha256(email)
        );

        String code = issuePasswordReset(user);
    ...
    @Transactional
    public AuthDtos.PasswordResetResponse resetPassword(
     ...
     *
     * Inactive accounts are rejected before authentication
     * and therefore cannot obtain a new JWT.
     */
    public TwoFactorDtos.LoginResponse login(
        ...
        checkAccountCanAuthenticate(user);
         ...
         *
         * This protects against the account status changing
         * during the authentication process.
         */
        checkAccountCanAuthenticate(user);

        if (user.isTwoFactorEnabled()) {
            return new TwoFactorDtos.LoginResponse(
                    "Bearer",
                    null,
                    null,
                    0,
        ...
        AuthDtos.TokenResponse tokens =
                issueTokens(principal);
     ...
     */
    public AuthDtos.TokenResponse verifyTwoFactorLogin(
            TwoFactorDtos.LoginVerifyRequest request
    ) {
        UUID userId =
        ...
        /*
         * Important:
         * A user may have been deactivated after the 2FA
         * challenge was created. Check status again before
         * issuing tokens.
         */
        checkAccountCanAuthenticate(user);
     ...
     *
     * An inactive account cannot refresh an existing session.
     */
    ...
    /**
     * Central account-status check.
     *
     * Keeping this in one method ensures login, 2FA and
     * refresh all apply exactly the same account rules.
     */
    private void checkAccountCanAuthenticate(User user) {
        if (
                ACCOUNT_STATUS_INACTIVE.equalsIgnoreCase(
                        user.getStatus()
                )
        ) {
            throw new IllegalStateException(
                    "Your account is inactive. Please contact OakPay support."
            );
        }

        if (
                !ACCOUNT_STATUS_ACTIVE.equalsIgnoreCase(
                        user.getStatus()
                )
        ) {
            throw new IllegalStateException(
                    "Your account is not available for authentication."
            );
        }
    }
    ...
    private String issueVerification(
        ...
        pending.setVerificationCodeHash(
                sha256(code)
        );

        pending.setVerificationExpiresAt(
                LocalDateTime.now().plus(VERIFY_TTL)
        );

        pending.setUpdatedAt(
                LocalDateTime.now()
        );
    ...
    private String issuePasswordReset(
            User user
    ) {
        passwordResetRepository.deleteByUserId(
                user.getId()
        );
        ...
        token.setTokenHash(sha256(code));
        token.setExpiresAt(
    ...
    private String developmentCode(String code) {
        return emailService.isDeliveryEnabled()
                ? null
                : code;
    }
    ...
    private AuthDtos.TokenResponse issueTokens(
            UserPrincipal principal
    ) {
        String accessToken =
